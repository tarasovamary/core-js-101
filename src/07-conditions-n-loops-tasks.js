...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const startInterval = a < b ? a : b;
  const endInterval = a < b ? b : a;
  const endFlag = isEndIncluded ? ']' : ')';
  return `${startFlag}${startInterval}, ${endInterval}${endFlag}`;
}
...
function isCreditCardNumber(num) {
  const ccn = num.toString();
  let sum = 0;
  const oddNum = ccn.length % 2;
  for (let i = 0; i < ccn.length; i += 1) {
    let ccnNum = Number(ccn[i]);
    if (i % 2 === oddNum) {
      ccnNum = (ccn[i] * 2 > 9) ? (ccn[i] * 2 - 9) : (ccn[i] * 2);
    }
    sum += ccnNum;
  }
  return sum % 10 === 0;
...
function isBracketsBalanced(str) {
  if (!str) return true;
  const regex = /{}|<>|\(\)|\[\]/;
  return str.match(regex) ? isBracketsBalanced(str.replace(regex, '')) : false;
}
